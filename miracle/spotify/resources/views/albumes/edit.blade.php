                        ...
                        @foreach ($usuarios as $usuario)
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="usuarios[]" value="{{ $usuario->id }}"
                                    {{ in_array($usuario->id, old('usuarios', $album->users->pluck('id')->toArray())) ? 'checked' : '' }}
                        ...
                        @foreach ($canciones as $cancion)
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="canciones[]" value="{{ $cancion->id }}"
                                    {{ in_array($cancion->id, old('canciones', $album->canciones->pluck('id')->toArray())) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring focus:ring-blue-500">
                ...
                <div class="mb-5">
                    ...
                    <div class="flex items-center justify-center mb-4">
                        <img src="{{ $album->imagen }}" alt="{{ $album->nombre }}"
                            class="rounded-lg shadow-lg max-w-60 h-auto">
                    </div>
                    <x-text-input name="imagen" type="file" id="imagen"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" />
