<x-app-layout>
    ...
    <div class="py-6">
        <div class="max-w-2xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('canciones.update', $cancion) }}" class="max-w-sm mx-auto"
                        ...
                        @foreach ($artistas as $artista)
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="artistas[]" value="{{ $artista->id }}"
                                    {{ in_array($artista->id, old('artistas', $cancion->artistas->pluck('id')->toArray())) ? 'checked' : '' }}
                        ...
                        @foreach ($albumes as $album)
                            <label class="flex items-center space-x-2">
                                <input type="checkbox" name="albumes[]" value="{{ $album->id }}"
                                    {{ in_array($album->id, old('albumes', $cancion->albumes->pluck('id')->toArray())) ? 'checked' : '' }}
                                    class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring focus:ring-blue-500">
