    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('ordenadores.update', $ordenador) }}" class="max-w-sm mx-auto">
                ...
                <div class="mb-5">
                    <div >
                        <label for="aula_id" class="block mb-2 text-sm font-medium text-gray-700">Selecciona un Aula</label>
                        <select id="aula_id" name="aula_id" class="block w-full p-2.5 text-sm bg-white border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
                            <option value="" disabled>Seleccione un aula...</option>
                            @foreach ($aulas as $aula)

                            <option value="{{ $aula->id }}" {{ $aula->id == $ordenador->aula->id ? 'selected' : ''}}>{{ $aula->nombre }}</option>
